...
class Problem:
    def __init__(self, N):
        ...
        self.N = N
        self.dx = 1 / N
        self.x = numpy.linspace(0, 1, N, endpoint=False) + self.dx / 2
    ...
    def set_initial_condition(self, y):
        ...
        density[:] = numpy.ones(self.N)
        momentum[:] = numpy.zeros(self.N)
        pressure = (
            (self.x < 0.1) * 1e3
            + (self.x >= 0.1) * (self.x < 0.9) * 1e-2
            + (self.x >= 0.9) * 1e2
        )
    ...
    def pack_boundary(self, index_j, density, momentum, energy):
        pack_1D_boundary_reflective(self.w, self.N, index_j, density, momentum, energy)
...
def main():
    ...
    if arg.method == "ssp":
        ...
        for index in range(3):
            q = get_subvector(vec_q, index)
            q[:] = numpy.random.rand(len(density))
        problem.estimator = SUNDomEigEstimator_Power(vec_q, 100, 5e-4, context)
        stepper = LSRKStepCreateSTS(problem.f, 0, y, context)
        LSRKStepSetDomEigEstimator(stepper.get(), problem.estimator)

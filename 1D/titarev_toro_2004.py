...
class Problem:
    def __init__(self, N):
        ...
        self.N = N
        self.dx = 10 / N
        self.x = numpy.linspace(-5, 5, N, endpoint=False) + self.dx / 2
    ...
    def set_initial_condition(self, y):
        ...
        density[:] = (self.x < -4.5) * 1.515695 + (self.x >= -4.5) * (
            1 + 0.1 * numpy.sin(20 * numpy.pi * self.x)
        )
        velocity = (self.x < -4.5) * 0.523346 + (self.x >= -4.5) * 0
        momentum[:] = density * velocity
        pressure = (self.x < -4.5) * 1.805 + (self.x >= -4.5) * 1
...
def main():
    ...
    parser.add_argument(
        "-nx", type=int, dest="nx", default=3000, help="mesh resolution in x-direction"
    )

...
class Problem:
    def __init__(self, N):
        ...
        self.N = N
        self.dx = 10 / N
        self.x = numpy.linspace(0, 10, N, endpoint=False) + self.dx / 2
    ...
    def set_initial_condition(self, y):
        ...
        density[:] = (self.x < 0.5) * (27 / 7) + (self.x >= 0.5) * (
            numpy.exp(-0.01 * numpy.sin(13 * self.x))
        )
        velocity = (self.x < 0.5) * (4 * numpy.sqrt(35) / 9) + (self.x >= 0.5) * 0
        momentum[:] = density * velocity
        pressure = (self.x < 0.5) * (31 / 3) + (self.x >= 0.5) * 1

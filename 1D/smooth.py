# https://doi.org/10.1016/j.jcp.2017.06.046
...
class Problem:
    def __init__(self, N):
        ...
        self.N = N
        self.dx = 2 / N
        self.x = numpy.linspace(-1, 1, N, endpoint=False) + self.dx / 2
    ...
    def set_initial_condition(self, y):
        ...
        density[:] = numpy.ones(self.x.shape) + 1 / 5 * numpy.sin(numpy.pi * self.x)
        velocity = numpy.sqrt(self.gamma) * density
        momentum[:] = density * velocity
        pressure = numpy.power(density, self.gamma)
    ...
    def pack_boundary(self, index_j, density, momentum, energy):
        pack_1D_boundary_periodic(self.w, self.N, index_j, density, momentum, energy)

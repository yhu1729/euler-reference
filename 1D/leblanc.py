...
class Problem:
    def __init__(self, N):
        ...
        self.N = N
        self.dx = 9 / N
        self.x = numpy.linspace(0, 9, N, endpoint=False) + self.dx / 2
    ...
    def set_initial_condition(self, y):
        ...
        density[:] = (self.x < 3) * 1 + (self.x >= 3) * 1e-3
        velocity = numpy.zeros(self.x.shape)
        momentum[:] = density * velocity
        energy[:] = (self.x < 3) * 0.1 + (self.x >= 3) * 1e-7
...
def main():
    ...
    ax[0, 0].semilogy(problem.x, density)
    ...
    for n in range(1, 4 + 1):
        ...
        ax[0, n].semilogy(problem.x, density)
